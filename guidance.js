(() => {
  ...
  const hints = {
    ...
    hirano_extra:['优先写设备/产线名称、你实际负责的调试环节，以及一个故障和结果；不会的不用补。','設備・ライン名、実際に担当した立上げ工程、不具合1件と結果を優先。不明な内容は不要です。'],
    ...
    canon_hirano_extra:['可按“露光机/模块 → 做过的精度调整 → 交付结果”写一行；想不起就留空。','「露光装置・モジュール → 精度調整 → 引渡し結果」の順に1行で可。不明なら空欄で構いません。'],
    changhong_extra:['优先写一个你能独立定位的典型故障，以及使用什么工具、换了什么部件、最后是否修复。','自分で切り分けた代表的な故障1件、使用工具、交換部品、修理結果を優先して記入します。']
  };
  const helpNodes=[];
